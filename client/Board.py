...
class Board:
    ...
    def clicked(self, pos):
        ...
        i, j = self.coord_from_pos(*pos)
        if self.isNotempty(i, j):
    ...
    def draw(self, win, dots,x, y):
        """Draws everything"""
        self.draw_board(win, x, y)
        self.draw_dots(win, dots, x, y)
    ...
    def draw_dots(self, win, moves, x,y):
        for move in moves:
            pygame.draw.circle(win, RED, (x + self.case_size * move[1] + self.case_size // 2,
                                          y + self.case_size * move[0] + self.case_size // 2), 5)
